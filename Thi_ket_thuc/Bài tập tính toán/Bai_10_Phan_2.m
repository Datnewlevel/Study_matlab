%% BÀI T?P 10: TÍNH GIÁ TR? BI?U TH?C A
...
for k = 1:n
    ...
    % S? m? c?a x là n, n-1, n-2, ... , 1
    % Khi k = 1, s? m? là n
    % Khi k = 2, s? m? là n-1
    % => Công th?c t?ng quát cho s? m? là: n - k + 1
    tuSo = x^(n - k + 1);
    
    % 2. Quy lu?t c?a M?U S?:
    % M?u s? là sqrt(1), sqrt(2), sqrt(3), ...
    % (L?u ý: s? h?ng ??u tiên x^n có m?u s? là 1, t?c là sqrt(1))
    % Hàm c?n b?c hai trong MATLAB là sqrt().
    % => Công th?c t?ng quát: mauSo = sqrt(k)
    mauSo = sqrt(k);
